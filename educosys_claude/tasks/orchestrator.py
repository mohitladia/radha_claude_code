...
class TaskOrchestrator:
    ...
    async def run(self, project_id: str) -> None:
        ...
        while True:
            ...
            # All tasks done — nothing pending and nothing in progress
            ...
            if not ready:
                if in_progress > 0:
                    ...
                    console.print(
                        "[yellow]⚠ No tasks are ready — some may be blocked by failed dependencies.[/yellow]"
                    )
    ...
    async def _execute(self, task: dict) -> None:
        ...
        try:
            # Fetch dependency results and inject into agent for context
...
async def handle_plan_command(goal: str) -> None:
    ...
    if project_id:
        ...
        # Re-plan loop: continue until human approves
...
def _print_final_summary(progress: dict[str, int]) -> None:
    """Print execution summary with counts by status."""
    completed = progress.get("completed", 0)
    failed    = progress.get("failed", 0)
