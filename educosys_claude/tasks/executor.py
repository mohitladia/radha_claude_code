...
# ──────────────────────────────────────────────────────────────────────
# Toolsets per task type — least privilege principle
# ──────────────────────────────────────────────────────────────────────

# Each task type gets a minimal, focused toolset.
# This limits blast radius and keeps agents focused.
...
def _parse_json_field(val) -> list:
    """Safely parse JSON string fields from DB (depends_on, output_files, etc.)."""
...
# ──────────────────────────────────────────────────────────────────────
# LLM-as-judge — verifies agent output against acceptance criteria
# ──────────────────────────────────────────────────────────────────────
...
async def _judge_task(task: dict, agent_output: str) -> _JudgeVerdict:
    ...
    Returns structured verdict with passed/score/reason.
    """
...
async def run_subtask_agent(task: dict, dep_outputs: list[dict] | None = None) -> str:
    ...
    Args:
        task: Task dict from SQLite (includes id, title, description, task_type,
              depends_on, output_files, acceptance_criteria, etc.)
        dep_outputs: List of {id, title, result} from completed dependencies

    Returns:
        Agent's final output string (summary of what was implemented)

    Raises:
        ValueError: If agent returns empty output or judge rejects it
    """
    provider = config["llm"]["provider"]
    ...
    # The project directory may be empty on first run — tell the agent to create
    # files directly rather than spending turns exploring an empty directory.
    user_message = (
        f"{task['description']}\n\n"
