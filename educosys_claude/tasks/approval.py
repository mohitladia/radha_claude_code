...
def present_plan_for_approval(plan: ExecutionPlan) -> ExecutionPlan | None:
    """
    Render the plan as a Rich table and prompt the user for approval.

    Interaction loop:
      [A]pprove → returns the plan (possibly modified)
      [M]odify  → edit a single task's description, re-render
      [R]eject  → returns None (caller re-plans with feedback)

    Continues until user approves or rejects.
    """
    while True:
        _render_plan(plan)
...
def _render_plan(plan: ExecutionPlan) -> None:
    """Render ExecutionPlan as a formatted Rich table."""
