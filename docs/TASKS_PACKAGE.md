...
# Dependency check
_all_deps_done(project_id, dep_ids) -> bool
```
...
└─────────────────────────────┬───────────────────────────────────┘
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  planner.create_plan(goal, extra_context)                       │
│  → ExecutionPlan (Pydantic validated)                           │
└─────────────────────────────┬───────────────────────────────────┘
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  approval.present_plan_for_approval(plan)                       │
│  → Rich table + [A]pprove / [M]odify / [R]eject loop            │
└─────────────────────────────┬───────────────────────────────────┘
                              ▼ (on Approve)
┌─────────────────────────────────────────────────────────────────┐
│  task_store.create_project(goal, plan)                          │
│  → INSERT projects + tasks (with JSON depends_on/output_files)  │
└─────────────────────────────┬───────────────────────────────────┘
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  recovery.recover(project_id)                                   │
│  → Reset any orphaned IN_PROGRESS → PENDING (retry) or FAILED   │
└─────────────────────────────┬───────────────────────────────────┘
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
│     if not ready:                                               │
│       if in_progress > 0: sleep(5); continue                   │
│       else: warn blocked; break                                 │
│                                                                 │
│     batch = ready[:max_concurrent]                              │
│     await asyncio.gather(*[_execute(t) for t in batch])         │
│                                                                 │
│   _execute(task):                                               │
│     if not store.claim_task(task.id): return                   │
│     dep_outputs = store.get_dep_results(task.depends_on)        │
│     result = await executor.run_subtask_agent(task, dep_outputs)│
│     store.complete_task(task.id, result)                        │
│     (on exception: store.fail_task → auto-retry via SQL CASE)   │
└─────────────────────────────┬───────────────────────────────────┘
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  /task_status  →  status.show_task_status()  →  Rich table      │
└─────────────────────────────────────────────────────────────────┘
```
...
├── test_task_store.py      # SQLiteTaskStore: transitions, queries, recovery
├── test_planner.py         # create_plan validation, Pydantic models
├── test_executor.py        # _build_system_prompt, toolset selection, judge
├── test_orchestrator.py    # get_ready_tasks DAG logic, batch dispatch
└── test_recovery.py        # recover() logic
```
...
Each module should have a module-level docstring (already present in most files):
...
| View logs | Check console output (Rich) + `logger` (file if configured) |
